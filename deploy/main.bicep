...
param pricingImage string
param pricingPort int = 8080
var pricingServiceAppName = 'pricing-app'
...
// Pricing App
module pricingService 'container-http.bicep' = {
  name: '${deployment().name}--${pricingServiceAppName}'
  dependsOn: [
    environment
  ]
  params: {
    ...
    location: location
    environmentName: environmentName
    containerAppName: pricingServiceAppName
    containerImage: pricingImage
    containerPort: pricingPort
    isPrivateRegistry: isPrivateRegistry
    minReplicas: minReplicas
    containerRegistry: containerRegistry
    registryPassword: registryPassword
...
module nodeService 'container-http.bicep' = {
  ...
  params: {
    ...
    env: [
      ...
      {
        name: 'PRICING_SERVICE_NAME'
        value: pricingServiceAppName
      }
    ]
    secrets: [
      {
...
output pricingFqdn string = pricingService.outputs.fqdn
